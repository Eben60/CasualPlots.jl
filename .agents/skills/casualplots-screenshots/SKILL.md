...
│   └── reports/                             # Verification reports (ignored by git)
...
Each screenshot has a detailed Markdown specification in `test/AgenticTesting/screenshots/specifications/<name>.md`. These specs are the single source of truth for both writing generator code and verifying output.
...
1. **Read the spec.** Use `view_file` on `test/AgenticTesting/screenshots/specifications/<name>.md`. DO NOT edit this file!
...
4. **Write the report.** Create (or **overwrite** if it already exists) a file named `report_<name>.md` in `test/AgenticTesting/screenshots/reports/`. The report must be **rewritten from scratch** on every iteration—never append to or patch a previous report. Use the 4-column table format (see [report_dataframe_source_selection.md](../../../../test/AgenticTesting/screenshots/reports/report_dataframe_source_selection.md) as the template):
