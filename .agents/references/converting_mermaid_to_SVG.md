...
1.  Scans the `.agents/mermaid/` directory for `.md` files.
2.  Extracts the Mermaid code block from each file.
3.  Applies fixes (e.g., converting escaped `\\n` to `\n` for proper newline rendering).
4.  Generates the corresponding SVG file in `.agents/diagrams/` via Kroki.

## Maintenance
To update the diagrams:
1.  Modify the Mermaid definitions in `.agents/mermaid/*.md`.
2.  Run the utility script:
    ```bash
    python3 tools/mermaid2SVG-utility.py
    ```
3.  The SVG files in `.agents/diagrams/` will be updated automatically.
