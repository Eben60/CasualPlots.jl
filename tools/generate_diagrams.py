...
diagrams = {
    ...
    "callback_sequence.svg": """sequenceDiagram
    participant User
    ...
    "state_transition.svg": """stateDiagram-v2
    [*] --> Idle: App Launched
...
os.makedirs("docs/src/Diagrams", exist_ok=True)

for output_file, content in diagrams.items():
    try:
        # Kroki expects utf-8 encoded string, compressed with zlib, then url-safe base64 encoded
        ...
        print(f"Downloading {output_file} from {url[:50]}...")
        with urllib.request.urlopen(url) as response:
            data = response.read()
            with open(f"docs/src/Diagrams/{output_file}", 'wb') as f:
                f.write(data)
