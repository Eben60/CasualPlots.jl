...
@testset "Identical units" begin
    using CasualPlots: initialize_app_state, unify_units!

    state = initialize_app_state()
    df = DataFrame(A = [1.0u"m", 2.0u"m"], B = [3.0u"m", 4.0u"m"])
    df_out = unify_units!(df, ["A", "B"], state)
...
@testset "Mixed compatible units (Metric)" begin
    ...
    df_out = unify_units!(df, ["A", "B", "C"], state)
...
@testset "Incompatible units" begin
    ...
    df_out = @test_logs (:warn, r"Incompatible physical dimensions") unify_units!(df, ["A", "B"], state)
...
@testset "Internal Mixed Compatible Units" begin
    using CasualPlots: unify_internal_column_units!

    df = DataFrame(A = Any[1.0u"m", 2.0u"cm", 3.0u"mm"])
    df_out = unify_internal_column_units!(df, ["A"])
