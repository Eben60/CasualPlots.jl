...
@testset "select_cols" begin
    using CasualPlots: select_cols

    # Create test dataframe
    df = DataFrame(A = 1:3, B = 4:6, C = 7:9, D = 10:12)

    result = select_cols(df)
    ...
    result = select_cols(df, cols=[:A, :C])
    @test size(result) == (3, 2)
    ...
    result = select_cols(df, cols=[:B])
    @test size(result) == (3, 1)
    @test names(result) == ["B"]

    # Both xcol and cols provided
    @test_throws ErrorException select_cols(df, xcol=2, cols=[:A, :B])

    # Non-existent column name
    @test_throws Exception select_cols(df, xcol=:NonExistent)
